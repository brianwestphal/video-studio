...
export const REQUIREMENT_COVERAGE = {
  ...
  "R-CB5": { status: "deferred", note: "Ollama backend: local model via app-driven constrained tool loop. Pure core built + 100% unit (desktop/sidecar/ollama-backend.mjs, tests/ollama-backend.test.mjs): OLLAMA_CAPABILITIES, buildOllamaMessages, parseModelReply, nextLoopAction (step-capped), toolResultMessage. The I/O loop (Ollama HTTP call + tool execution through decide()) + host dispatch on config.agentBackend + live verification remain (VS-94)" },
  ...
  "R-CB7": { status: "unit", tests: ["sidecar-protocol.test.mjs"], note: "structured cut-plan pipeline (all pure + unit): extractCutPlan (pull plan JSON from the agent's result text) -> validateCutPlan (validate/normalize) -> unknownPlanMembers (reject a plan referencing a non-existent/placeholder angle) -> cutPlanToSwitches (writable switches.json with version+groupId). The host lands it (agent-run result -> tryLandCutPlan writes switches.json, returns landedCut) + the frontend opens Review on it, else falls back to design-cut — that wiring is the I/O edge (manual §15.18, VS-96)" },
