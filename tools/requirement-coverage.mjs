...
export const REQUIREMENT_COVERAGE = {
  ...
  "R-CB7": { status: "unit", tests: ["sidecar-protocol.test.mjs"], note: "validateCutPlan (structured cut-plan validation + normalize/sort) + cutPlanToSwitches (plan -> writable switches.json doc with version+groupId, the Auto->Review bridge) — agent.mjs; driving the live agent to emit the plan + writing the file is the I/O tail (VS-91/96)" },
