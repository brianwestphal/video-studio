...
export const REQUIREMENT_COVERAGE = {
  ...
  "R-KERF1": { status: "gate", note: "desktop and review UI Kerf bundles plus Chromium primary flows run in npm run check (VS-119/120/122)" },
  ...
  "R-KERF3": { status: "gate", note: "both clients use signals + one mount; preserved iframe/media boundaries run in Chromium (VS-119/120/122)" },
  "R-KERF4": { status: "unit", tests: ["review-components.test.tsx", "desktop-app.test.tsx"], note: "desktop stages and review segments/candidates render with stable data-key identities" },
  "R-KERF5": { status: "gate", note: "both clients delegate from stable roots; Chromium covers actions, dialogs, iframe and media flows" },
  "R-KERF6": { status: "unit", tests: ["review-components.test.tsx", "desktop-app.test.tsx"], note: "dynamic UI markup is escaped typed Kerf TSX; preserved timeline uses toElement" },
  ...
  "R-KERF8": { status: "gate", note: "component/state tests plus desktop and review Chromium primary flows run in npm run check" },
  ...
  "R-APP1": { status: "manual", note: "Tauri shell + vanilla webview over the unchanged pipeline — built, compiles + runs (npm run desktop:dev) (VS-90)" },
  "R-APP1a": { status: "manual", note: "assume-nothing doctor probes Node + tools; detect + guide — built (Setup screen) (VS-90)" },
  ...
  "R-CB6": { status: "unit", tests: ["sidecar-protocol.test.mjs", "codex-backend.test.mjs"], note: "eventToFeedEntry — outcome-oriented feed summaries; suppresses internal telemetry and exact commands, exposes at most safe tool metadata (VS-91/108); the live SDK run is host I/O" },
  ...
  "R-PERM5": { status: "manual", note: "native prompt offers Allow once / Always allow this kind / Deny; Always persists the unit-tested project-scoped category rule (manual-test-plan §15.22, VS-97)" },
  ...
  "R-PERM7": { status: "manual", note: "decide is unit-tested; live canUseTool auto-allows safe normalized in-project paths and parks ask decisions on the correlated native prompt (manual-test-plan §15.22, VS-97)" },
  "R-PERM7a": { status: "manual", note: "SDK sandbox auto-approval disabled so Bash reaches canUseTool; PreToolUse independently blocks explicit denies — adversarially verified (manual-test-plan §15.16) (VS-97)" },
  "R-PERM8": { status: "unit", tests: ["sidecar-protocol.test.mjs"], note: "deriveAllowedTools from allow-by-default categories, kept in sync (pure). Intentionally NOT wired into runAgentRun's options.allowedTools: a bare allow-list entry auto-approves a tool before canUseTool (SDK CLAUDE_SDK_CAN_USE_TOOL_SHADOWED), collapsing our two-point gate; the safe categories already 'allow' in canUseTool so there's no friction to save (VS-92/96)" },
  "R-PERM9": { status: "manual", note: "decide never gates questions; Claude onUserDialog routes them to the native picker (manual-test-plan §15.23, VS-97)" },
