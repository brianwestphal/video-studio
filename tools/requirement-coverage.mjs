...
export const REQUIREMENT_COVERAGE = {
  ...
  "R-CB7": { status: "unit", tests: ["sidecar-protocol.test.mjs"], note: "structured cut-plan pipeline (all pure + unit): extractCutPlan (pull the plan JSON from the agent's result text) -> validateCutPlan (validate/normalize) -> cutPlanToSwitches (writable switches.json with version+groupId). The host lands it (agent-run result -> tryLandCutPlan writes switches.json, returns landedCut) + the frontend opens Review on it, else falls back to design-cut — that wiring is the I/O edge (manual §15.13, VS-96)" },
  ...
  "R-PERM8": { status: "unit", tests: ["sidecar-protocol.test.mjs"], note: "deriveAllowedTools from allow-by-default categories (pure); now wired into runAgentRun's options.allowedTools so safe read/media/write skip the canUseTool round-trip while the PreToolUse hook stays the authoritative gate — verified in a live agent run (VS-92/96)" },
