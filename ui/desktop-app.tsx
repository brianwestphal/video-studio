...
function ExportScreen(): SafeHtml { return <section class="screen" data-screen="export" hidden={screen.value !== "export"}><Header title="Export" sub="Turn your designed cut into a finished file. Open a project first."/><div class="export-cards">{each(EXPORTS.map((item) => ({ ...item })), (item) => { const state = exportsState.value[item.kind] ?? { status: "ready", running: false }; return <div class="export-card" data-key={item.kind} data-kind={item.kind}><div class="export-title">{item.title}</div><div class="export-desc">{item.desc}</div><button class="btn export-run" disabled={state.running} {...ACTIONS.exportRun.attrs}>Export</button>{state.running ? <button class="btn small export-cancel" {...ACTIONS.exportCancel.attrs}>Cancel</button> : ""}<div class="export-status">{state.status}</div>{state.outPath ? <button class="btn small export-reveal" data-path={state.outPath} {...ACTIONS.reveal.attrs}>Reveal in Finder</button> : ""}</div>; })}</div></section>; }
...
declare global { interface Window { __TAURI__?: { core: { invoke: Invoke }; event: { listen: Listen } } } }
...
function goto(key: ScreenKey): void { screen.value = key; if (key === "permissions") loadConfig(); }
