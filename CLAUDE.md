...
- **Unit tests** (`tests/**/*.test.ts`, `tests/**/*.test.mjs`): [Vitest](https://vitest.dev) with v8 coverage. They cover the **pure, side-effect-free logic** extracted out of the I/O modules: `src/scene-math.ts` (fps parsing, frame↔timecode, scene merging) and `tools/caption-format.mjs` (arg parsing, SVG/HTML assembly). These two files are held to **100% lines/branches/functions/statements** (`vitest.config.ts` `coverage.include` + `thresholds`). When you extract more pure logic, add it to `coverage.include` and test it to 100%.
