...
- **UI foundation**: browser TypeScript/TSX is checked by `tsconfig.ui.json`, bundled by `npm run build:ui` into `dist/ui/`, linted with `eslint-plugin-kerfjs`, and imports shared primitives through `ui/kerf.ts`.
