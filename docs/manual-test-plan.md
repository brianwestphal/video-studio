...
| 7.12 | Export with `--start <sec>` set past where the master audio leads the first video frame (e.g. the latest member offset), then import into FCP | The edit begins on live footage (no black head), and the **master audio is in sync** with the picture — FCP plays a pre-video audio lead ahead of the picture, so trimming to where the cameras roll locks A/V. The flat preview (§7.11) with the same `--start` matches the FCP playback. |
