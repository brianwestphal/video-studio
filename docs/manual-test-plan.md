...
| 15.13 | **Design** stage: click a preset (e.g. Teaser) | The prompt box fills with the preset text. **Make my cut** shows a "connect an AI agent" note (the Auto lane is agent-powered; live backend pending VS-91). **Open the timeline** proposes an auto cut via `propose-switches` when there's none (status updates), refreshes the rail, and lands on Review; with an existing cut it jumps straight to Review. |
