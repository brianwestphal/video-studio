...
| 13.22 | (VS-120/VS-122) Inspect the loaded review page and exercise pick/audio/fullscreen, scrub/play, timeline open, split/add, re-propose, and Save | The page loads only `review-entry.js`, its root reports `data-ui-runtime="kerfjs"`, controls keep working after reactive picks/re-propose, video playback state survives keyed renders, and only one audio source plays. The primary flow also runs in Chromium via `npm run test:browser`. |
...
| # | Action | Expect |
|---|--------|--------|
| 15.1 | `npm run desktop:dev` | The "Video Studio" window opens on the **Setup** screen; the five-stage rail shows Setup active, New Project reachable, and Analyze/Design/Export **locked** (greyed, not clickable). There is no redundant Review rail item. |
...
| 15.20 | (VS-110) Enter **Export** with a completed cut and `audio-events.json` | A lightweight 640×360 proxy renders automatically, then appears as a native scrubbable video. The post-edit audio map lists vocal/instrumental/quiet/section cues; clicking one seeks and plays the proxy. For a single-source montage, cue times follow the concatenated output rather than source time. Return to Design, change the cut, then re-enter Export: the proxy regenerates. |
| 15.22 | (VS-109) Export a single-source cut with hard edits through loud, non-zero waveform samples; also export an editor-handoff cut with kept segment audio | Playback crosses each hard edit without an audible pop. The flat render and kept-audio segment commands apply bounded 5 ms fades. Transitioned cuts still use their audio crossfade, and continuous multi-camera master audio remains unchanged. |
| 15.12 | Open a multi-cam project with a `switches.json`; in **Design**, click **Open timeline editor** | The review UI loads inline in Design (no separate browser window — `review-switches --no-open`), flagged cuts first, synced angle player + timeline. Picking/splitting + Save writes back to `switches.json`; reopening reuses the same server. |
| 15.13 | **Design** stage: click a preset (e.g. Teaser) | The prompt box fills with the preset text. **Open timeline editor** proposes an auto cut via `propose-switches` when needed, then expands the editor inline. A single-source cut explains that timeline editing is multi-camera-only and remains ready for Export. |
| 15.18 | (VS-96/113) **Design → Auto** on a multi-cam project: type a prompt (e.g. "punchy 15s highlight") → **Make my cut** | The activity feed streams the live agent run. When the agent ends with a `{ switches: [...] }` cut plan, the host lands `switches.json` and the app opens **Export**. The optional timeline editor remains in Design. Invalid plans fall back to `design-cut`. |
| 15.19 | (VS-94) **Ollama backend**: with Ollama running + a model installed, send `agent-run` with `backend: "ollama"` (or set `agentBackend: "ollama"` in the app config) on a multi-cam project | The sidecar runs the app-driven constrained tool loop against the local model: the feed shows `Local model <name>`, then `Tool: read_file` / `Tool: propose_baseline` as the model requests them — **each gated through the same `decide()` choke point as Claude** (a denied tool returns "Denied by video-studio's safety policy" to the model). On a final `{ switches: [...] }` reply the host lands `switches.json` (same `tryLandCutPlan` + memberId guard). Verified against `gemma4:12b`: read_file → propose_baseline → landed a valid cut. No auth (local). The in-app backend-selector UI is a follow-up. |
| 15.21 | (VS-104) **Single-source Auto lane**: on a single-video project (`sources.json`, no `multicam.json`), Design → type a prompt → **Make my cut** | The prompt asks the agent for a **clip-range** plan (`{ clips: [{ in, out }] }`) rather than angle switches. On a valid plan the host lands **`cut.json`** (project meta from `sources.json`, clips over the single video) via `landSingleSourcePlan` and the app advances to **Export**; an invalid/absent plan falls back to the deterministic `design-cut` baseline. Verified live against the Claude backend (agent read `sources.json` → clip plan → landed `cut.json`). |
| 15.20 | (VS-93) **Codex backend**: with Codex logged in (`codex login status`), send `agent-run` with `backend: "codex"` on a multi-cam project | The sidecar spawns `codex exec --json` in a **read-only sandbox** (the permission boundary — Codex may read the project but never writes/executes) and normalizes its JSONL stream to the shared feed (`Session started`, `Running a command`, `Codex …`). Codex is constrained to the cut-plan `--output-schema`; the host reads its final message and lands `switches.json` via the same `tryLandCutPlan` + memberId guard. Verified against ChatGPT auth: session → gated command (read multicam.json) → structured plan → landed a valid cut (groupId `byam`, real memberIds). Session resume (`codex exec resume`) + the backend-selector UI are follow-ups. |
...
| 15.17 | (VS-99/113) Single video: New Project → open a folder with ONE video → Analyze this footage → Analyze → Design → Auto "Make my cut" | `design-cut` writes `cut.json`; Design completes and **Export** unlocks directly. Export MP4 flat-renders `cut.json` (ffmpeg trim+concat) → `exports/cut.mp4`; 9:16 pads to 1080×1920; FCPXML runs `export-project`. |
...
| 15.22 | (VS-97) In Design → Auto, ask for a cut that needs project inspection, then one that attempts an unrecognized shell/network action | Relative in-project Read/Grep and recognized pipeline/ffmpeg calls run silently. The risky call pauses the agent and opens an app dialog showing the category/action with **Allow once / Always allow this kind / Deny**. Allow once resumes only that call; Always resumes and creates a project-scoped remembered approval visible under Permissions; Deny blocks the call but lets the agent continue. Cancelling the run/dialog leaves no stale prompt. |
| 15.23 | (VS-97) Give the Auto-lane agent an instruction that requires it to ask a structured `AskUserQuestion` choice | The app opens a friendly option picker; Continue returns the selected answer and the same agent run resumes. Cancel returns a cancelled answer without hanging the sidecar. |
| 15.24 | (VS-112) Open or create several project folders, relaunch the app, then return to **New Project** | Recent projects persist across launches, newest first, deduplicated and capped. Each row shows its folder name + full path; clicking reopens it and promotes it to the front. With no history, the section is hidden. |
| 15.14 | With `ANTHROPIC_API_KEY` set, **Make my cut** (Auto lane) with a prompt | A live activity feed streams from Claude with outcome-oriented summaries such as "Loading the editing workflow" and "Inspecting analyzed footage", ending in "Done". It does not show bare "Using Bash/Skill", exact commands, or internal telemetry such as `thinking_tokens`; file activity exposes at most a basename. Every escalated tool the agent tries is gated by video-studio's policy (`decide`): allowed silently, or denied with our message. No key → a "Claude isn't connected" note. Drive the sidecar directly: `echo '{"type":"request","id":1,"step":"agent-run","params":{"prompt":"…"}}' \| ANTHROPIC_API_KEY=… node desktop/sidecar/host.mjs`. |
