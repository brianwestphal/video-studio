...
| 13.22 | (VS-120) Inspect the loaded review page and exercise pick/audio/fullscreen, scrub/play, timeline open, split/add, re-propose, and Save | The page loads `review-entry.js`, its root reports `data-ui-runtime="kerfjs"`, controls keep working after reactive picks/re-propose, video playback state survives keyed renders, and only one audio source plays. The inline legacy source is inert (`type="text/plain"`). |
