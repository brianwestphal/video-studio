...
| 7.11 | `node tools/render-multicam-preview.mjs <multicam.json> --switch 0=<id> --switch <t>=<id2> [--width <w> --height <h>] --out preview.mp4`, then play it (the BYAM fixture is `external/multi-cam/BYAM-multicam-preview.mp4`) | A flat MP4 of the **same** angle cut as the §7.9 FCPXML: video cuts between the synced angles at the switch points, the master audio plays continuously underneath, and a chosen angle with no footage yet at a cut shows **black** until its camera rolled (matching FCP's gap). Use it to eyeball the edit and sanity-check sync without FCP. |
