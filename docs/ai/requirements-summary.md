...
- **Edit awareness / auto multi-cam cutting (Shipped; BYAM demo manual)** — three specs make the
  ...
  **shot-length policy is shipped (VS-62)**: default max 8s / min 0.5s with an
  ...
  (wide/medium/close) is now part of the saliency vision schema (R-RUI7); wiring the
  review UI's save to re-propose downstream is VS-67. Follow-up: a saliency performer/instrument
