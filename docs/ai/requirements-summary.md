...
| Desktop app (VS-76 initiative) | [`desktop-app.md`](../desktop-app.md), [`desktop-app-agent-bridge.md`](../desktop-app-agent-bridge.md), [`desktop-app-permissions.md`](../desktop-app-permissions.md) | **Partial** — a native Tauri shell over the existing pipeline (glassbox pattern: Rust shell → long-lived Node sidecar → vanilla webview). Specs complete for the three foundation pieces (forks settled 2026-07-03): app shell / project model / stage nav + sidecar host (R-APP, VS-80), a **pluggable/optional** AI agent bridge for the Auto lane — Claude / Codex / Ollama (R-CB1–11, VS-83), and the app-owned category-based permission & safety layer (R-PERM1–12, VS-85). **Built:** the Node sidecar core — `desktop/sidecar/protocol.mjs` (R-APP12) + `steps.mjs` (R-APP13), pure + 100% unit (VS-90). Remaining: the native Tauri shell + screens, project model, doctor, the agent bridge + backends, and the permission layer (VS-90/91/92/93/94 + the screen tickets). |
