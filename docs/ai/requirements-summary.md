...
| Desktop app (VS-76 initiative) | [`desktop-app.md`](../desktop-app.md), [`desktop-app-agent-bridge.md`](../desktop-app-agent-bridge.md), [`desktop-app-permissions.md`](../desktop-app-permissions.md) | **Partial** — native Tauri shell, five-stage project rail, sidecar, core screens, and Claude/Codex/Ollama Auto backends are built. Completed cuts go Design → Export with optional inline timeline editing (VS-113); Export automatically renders an embedded 640×360 scrubbable proxy with a cut-rebased audio-event map (VS-110); and recent projects persist and reopen from New Project (VS-112). The app-owned permission layer is shipped end-to-end (VS-92/97). Remaining work is tracked in the screen/polish tickets. |
...
| Kerfjs UI standard | [`kerfjs-ui.md`](../kerfjs-ui.md) | **Implemented** (VS-115/119–122) — both interactive surfaces use typed keyed components, one reactive mount per client, delegated actions, and preserved iframe/media DOM; component tests and both Chromium primary flows run in `npm run check`. |
...
- **Desktop app (VS-76; partial implementation)** — a native app front door over the
  ...
  The Export lane includes an automatically regenerated 640×360 scrubbable cut proxy and a
  clickable post-edit audio-event map rebased through single-source cuts (VS-110, R-EX6/7).
