...
│   ├── desktop-app.md          # DESIGN: Tauri shell + project model + stage nav + Node sidecar host (R-APP, VS-76/80)
│   ├── desktop-app-claude-bridge.md  # DESIGN: Auto lane via Claude Agent SDK — structured events → UI, canUseTool (R-CB, VS-83)
...
| desktop app (VS-76, design only) | `docs/investigations/ui-app.md` (concept + roadmap + wireframe) → `docs/desktop-app.md` (shell/project model/stage nav/sidecar host, R-APP, VS-80) + `docs/desktop-app-claude-bridge.md` (Auto lane via Claude Agent SDK, R-CB, VS-83) + `docs/desktop-app-permissions.md` (app-owned permission layer, R-PERM, VS-85); all `deferred` in the coverage manifest until built |
